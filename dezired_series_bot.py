...
# Set up logging
logging.basicConfig(filename="/app/data/bot.log", level=logging.INFO, format="%(asctime)s - %(levelname)s - %(message)s")
...
timestamp_data = {}
try:
    if os.path.exists("/app/data/scanned_data.pkl"):
        with open("/app/data/scanned_data.pkl", "rb") as f:
            scanned_data = pickle.load(f)
            logger.info("Loaded scanned_data.pkl")
    if os.path.exists("/app/data/timestamp_data.pkl"):
        with open("/app/data/timestamp_data.pkl", "rb") as f:
...
def clean_old_entries():
    ...
    try:
        with open("/app/data/scanned_data.pkl", "wb") as f:
            pickle.dump(scanned_data, f)
            logger.info("Saved scanned_data.pkl")
        with open("/app/data/timestamp_data.pkl", "wb") as f:
